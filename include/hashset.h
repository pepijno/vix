...
#define HASHSET_DEFS(__type, __name)                                    \
    ...
    struct HASHSET_NAME(__name) {                                       \
        ...
        struct HASHSET_VALUE_HASH_PAIR(__name) * data;                  \
                                                                        ...
                                                                        \
    struct HASHSET_NAME(__name) CONCAT(HASHSET_NAME(__name), _new)(     \
        struct allocator allocator[static const 1]                      \
    );                                                                  \
                                                                        \
    void CONCAT(HASHSET_NAME(__name), _add)(                            \
        struct HASHSET_NAME(__name) __name[static const 1],             \
        __type const element                                            \
    );                                                                  \
                                                                        \
    bool CONCAT(HASHSET_NAME(__name), _contains)(                       \
        struct HASHSET_NAME(__name) const __name, __type const element  \
    );                                                                  \
                                                                        \
    struct HASHSET_NAME(__name)                                         \
        CONCAT(HASHSET_NAME(__name), _copy)(struct HASHSET_NAME(__name) \
                                                const __name);
...
#define HASHSET_IMPL(__type, __name, __hash_fn, __is_equals_fn)                                                                 \
    ...
    static struct CONCAT(__name, __LINE__)                                                           \
        CONCAT(HASHSET_NAME(__name), _find_new_position)(                                            \
            struct HASHSET_NAME(__name) const __name, u32 const hash,                                \
            __type const element                                                                     \
        ) {                                                                                          \
