...
#define HASHMAP_DEFS(__key_type, __value_type, __name)                 \
    ...
    struct HASHMAP_NAME(__name) {                                      \
        ...
        struct KV_PAIR_HASH_NAME(__name) * data;                       \
                                                                       ...
                                                                       \
    struct HASHMAP_NAME(__name) CONCAT(HASHMAP_NAME(__name), _new)(    \
        struct allocator allocator[static const 1]                     \
    );                                                                 \
                                                                       \
    __value_type CONCAT(HASHMAP_NAME(__name), _add)(                   \
        struct HASHMAP_NAME(__name) __name[static const 1],            \
        __key_type const key, __value_type const value                 \
    );                                                                 \
                                                                       \
    __value_type CONCAT(HASHMAP_NAME(__name), _find_or_insert_new)(    \
        struct HASHMAP_NAME(__name) __name[static const 1],            \
        __key_type const key                                           \
    );                                                                 \
                                                                       \
    struct CONCAT(HASHMAP_NAME(__name), _search_result) {              \
        __value_type value;                                            \
        bool found;                                                    \
    };                                                                 \
                                                                       \
    struct CONCAT(HASHMAP_NAME(__name), _search_result)                \
        CONCAT(HASHMAP_NAME(__name), _find)(                           \
            struct HASHMAP_NAME(__name) __name, __key_type const key   \
        );                                                             \
                                                                       \
    bool CONCAT(HASHMAP_NAME(__name), _contains)(                      \
        struct HASHMAP_NAME(__name) const __name, __key_type const key \
    );
...
#define HASHMAP_IMPL(                                                         \
    ...
    static struct CONCAT(__name, __LINE__)                                    \
        CONCAT(HASHMAP_NAME(__name), _find_new_position)(                     \
            struct HASHMAP_NAME(__name) const __name, u32 const hash,         \
            __key_type const key                                              \
        ) {                                                                   \
    ...
    struct CONCAT(HASHMAP_NAME(__name), _search_result)                       \
        ...
        while (el.hash != 0) {                                                \
            if (__is_equals_fn(el.pair.key, key)) {                           \
                return (struct CONCAT(HASHMAP_NAME(__name), _search_result)){ \
                    .value = (__name).data[position].pair.value,              \
                    .found = true,                                            \
                };                                                            \
            }                                                                 \
            position = (position + increase) & ((__name).capacity - 1);       \
    ...
    __value_type CONCAT(HASHMAP_NAME(__name), _find_or_insert_new)(           \
        ...
        auto value = CONCAT(HASHMAP_NAME(__name), _find)(*(__name), key);     \
        if (!value.found) {                                                   \
    ...
    bool CONCAT(HASHMAP_NAME(__name), _contains)(                             \
        ...
        return CONCAT(HASHMAP_NAME(__name), _find)((__name), key).found;      \
    }
